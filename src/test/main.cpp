...
/***************************************************************************
    ...
    Controls execution of the core "test" board system.
...
{
private:
	DISABLE_COPYING(test_machine_manager);
	// construction/destruction
	test_machine_manager(emu_options &options, osd_interface &osd) : machine_manager(options, osd) { }
public:
	static test_machine_manager *instance(emu_options &options, osd_interface &osd)
	{
		if (!m_manager)
		{
			m_manager = new test_machine_manager(options, osd);
		}
		return m_manager;
	}

	static test_machine_manager *instance() { return m_manager; }

	~test_machine_manager() { delete m_manager;  m_manager = nullptr; }

	int execute()
	{
		auto system = &GAME_NAME(test);
...
private:
	static test_machine_manager* m_manager;
...
test_machine_manager* test_machine_manager::m_manager = nullptr;
...
int emulator_info::start_frontend(emu_options &options, osd_interface &osd, std::vector<std::string> &args)
{
	...
	test_machine_manager::instance(options,osd)->start_http_server();
	test_machine_manager::instance(options,osd)->execute();
	return 0;
}
...
const char * emulator_info::get_appname_lower() { return "test"; }
